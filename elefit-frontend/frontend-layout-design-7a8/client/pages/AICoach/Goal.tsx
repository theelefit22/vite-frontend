...
export default function Goal() {
  const [goal, setGoal] = useState<string>("Lose 6 kg in 3 months");
  ...
  return (
    <div className="relative min-h-screen w-full bg-black bg-opacity-80">
      ...
      {/* Content */}
      <div className="relative z-10 flex min-h-[calc(100vh-154px)] items-center justify-center px-4 pb-24">
        <div className={`flex flex-col w-[353px] items-center gap-[38px] relative transition-all duration-500 ease-out md:transition-none ${isMounted ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0 md:translate-y-0 md:opacity-100'}`}>
          <div className="flex-col h-[131px] items-start gap-6 flex relative self-stretch w-full">
            <header className="items-center gap-2.5 flex-[0_0_auto] flex relative self-stretch w-full">
              <h1 className="relative w-fit [font-family:'Inter-Bold',Helvetica] font-bold text-white text-xl tracking-[0.40px] leading-[28.0px] whitespace-nowrap">
                What&apos;s your fitness goal?
              </h1>

              <span
                className="relative w-fit mt-[-1.00px] [font-family:'Inter-SemiBold',Helvetica] font-semibold text-[23.5px] tracking-[0.47px] leading-[32.9px] text-black whitespace-nowrap"
            ...
            <div className="h-[74px] items-center gap-2.5 px-[22px] py-[26px] rounded-2xl border border-solid border-[#454545] flex relative self-stretch w-full bg-[#212121] bg-opacity-90">
              <input
                type="text"
                value={goal}
                onChange={(e) => setGoal(e.target.value)}
                placeholder="Lose 6 kg in 3 months"
                className="w-full bg-transparent text-white placeholder:text-[#888888] text-base focus:outline-none"
              />
          ...
          <button
            ...
            className="flex h-12 items-center justify-center gap-1 px-4 py-2 relative self-stretch w-full bg-[#ccd853] rounded-[30px] cursor-pointer hover:bg-[#b8c44a] transition-colors focus:outline-none focus:ring-2 focus:ring-[#ccd853] focus:ring-offset-2"
            aria-label="Create my fitness plan"
          >
