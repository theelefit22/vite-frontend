...
export const Frame = (): JSX.Element => {
  return (
    <div className="flex flex-col w-[402px] items-end gap-[9px] relative">
      ...
      <div className="flex flex-col h-[7px] items-start gap-2.5 relative self-stretch w-full bg-[#292929] rounded-[30px]">
        <div className="relative w-[216px] h-[7px] bg-[#ccd853] rounded-[30px_0px_0px_30px]" />
      </div>
    </div>
  );
};
  ...
  return (
    <div className="relative min-h-screen w-full bg-black bg-opacity-80">
      ...
      <div className="relative z-10 flex min-h-[calc(100vh-154px)] items-start justify-center px-4 pt-8 pb-24">
        <div className="flex flex-col w-full max-w-md items-center gap-[25px] relative">
          {/* Progress Indicator */}
          <div className={`w-full transition-all duration-500 ease-out md:transition-none ${isMounted ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0 md:translate-y-0 md:opacity-100'}`}>
            <Frame />
          </div>
          
          <div className={`flex flex-col items-end gap-[26px] relative self-stretch w-full flex-[0_0_auto] transition-all duration-500 ease-out delay-100 md:transition-none ${isMounted ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0 md:translate-y-0 md:opacity-100'}`}>
            <div className="flex flex-col items-start gap-9 relative self-stretch w-full flex-[0_0_auto]">
              ...
              <div className="flex gap-2.5 relative self-stretch w-full flex-[0_0_auto]">
                {[
                  ...
                  <button
                    ...
                    className={`relative flex flex-1 flex-col items-center gap-1 rounded-2xl border p-3 transition-colors min-h-[100px] ${
                      helpType === option.id
                        ? 'border-primary bg-[#212121] bg-opacity-90'
                        : 'border-[#454545] bg-[#212121] bg-opacity-90 hover:border-[#666]'
                    }`}
                  >
                    {option.badge && (
                    ...
                    <span className="text-xs font-semibold text-white text-center">{option.label}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="flex flex-col items-start gap-3.5 relative self-stretch w-full flex-[0_0_auto]">
              <h2 className="relative self-stretch mt-[-1.00px] [font-family:'Inter-Regular',Helvetica] font-normal text-[#fbffdb] text-sm tracking-[0] leading-[normal]">
                Activity Level
              </h2>
              <div className="grid grid-cols-2 gap-2.5 relative self-stretch w-full flex-[0_0_auto]">
                {activityLevels.map((level) => (
                  <button
                    key={level.id}
                    type="button"
                    onClick={() => setActivityLevel(level.id as ActivityLevel)}
                    className={`flex flex-col items-center gap-1 rounded-2xl border p-2 transition-colors min-h-[110px] ${
                  ...
                  >
                    ...
                    <span className="text-xs font-semibold text-white text-center">{level.label}</span>
                    <span className="text-[10px] text-[#898989] text-center leading-tight">{level.desc}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="flex flex-col items-start gap-3.5 relative self-stretch w-full flex-[0_0_auto]">
              <div>
                ...
                <p className="relative self-stretch [font-family:'Inter-Medium',Helvetica] font-medium text-[#ccd853] text-xs tracking-[0.24px] leading-[16.8px]">
                  {workoutDays} days - balanced & sustainable
                </p>
              </div>
              <input
                type="range"
            ...
            <div className="flex flex-col items-start gap-3.5 relative self-stretch w-full flex-[0_0_auto]">
              <div>
                <label className="relative self-stretch mt-[-1.00px] [font-family:'Inter-Regular',Helvetica] font-normal text-[#fbffdb] text-sm tracking-[0] leading-[normal]">
                  Dietary preferences
                </label>
                <div className="flex h-12 items-center justify-center gap-2.5 px-4 py-[11px] relative self-stretch w-full rounded-lg overflow-hidden border border-solid border-[#454545] bg-[#212121] bg-opacity-90">
                  <input
                    type="text"
                    value={dietary}
                    onChange={(e) => setDietary(e.target.value)}
                    placeholder="Eg., Vegetarian, no dairy"
                    className="relative flex-1 [font-family:'Inter-Regular',Helvetica] font-normal text-[#595959] text-sm tracking-[0] leading-[normal] placeholder:text-[#595959] bg-transparent w-full focus:outline-none"
                  />
                </div>
              </div>
              <div className="flex flex-wrap gap-2">
                {dietTags.map((tag) => (
                  <button
                    ...
                    className={`rounded-lg px-3 py-2 text-xs transition-colors min-h-[44px] flex items-center ${
                      dietaryTags.includes(tag)
                        ? 'bg-[#0088ff] text-[#e4e4e4]'
                        : 'bg-[#212121] text-[#e4e4e4] hover:bg-[#333]'
                    }`}
                  >
                    {tag}
                  </button>
          ...
          <button
            ...
            className={`flex h-12 items-center justify-center gap-1 px-4 py-2 relative self-stretch w-full bg-[#ccd853] rounded-[30px] cursor-pointer hover:bg-[#b8c44a] transition-colors focus:outline-none focus:ring-2 focus:ring-[#ccd853] focus:ring-offset-2 transition-all duration-500 ease-out delay-200 md:transition-none ${isMounted ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0 md:translate-y-0 md:opacity-100'}`}
            aria-label="Save profile and proceed"
          >
